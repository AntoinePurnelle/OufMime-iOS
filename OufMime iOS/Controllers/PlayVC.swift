...
class PlayVC: StoryboardedVC {
  ...
  func initTimer() {
    ...
    timer = Timer.scheduledTimer(withTimeInterval: 1.0, repeats: true) { timer in
      ...
      if vm.timerCurrentTime == 4 {
        self.play(sound: "timer.wav", isTimerSound: true)
      }
  ...
  private func playWord(found: Bool) {
    if let coordinator = coordinator {
      coordinator.viewModel.playWord(wasFound: found)
  ...
  func play(sound soundName: String, isTimerSound: Bool = false) {
    ...
    queue.async {
      ...
      do {
        ...
        if (isTimerSound) {
          self.timerPlayer = try AVAudioPlayer(contentsOf: url, fileTypeHint: AVFileType.wav.rawValue)
